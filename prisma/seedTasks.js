...
async function getRandomRootCause(machineId, typeOfFailureId) {
  ...
  if (rootCauses.length === 0) {
    throw new Error(`No root causes found for Machine Type ID: ${machineTypeId} and Failure Type ID: ${typeOfFailureId}`);
  }
...
async function seedTasks() {
  for (let i = 0; i < 5; i++) {
    ...
    const requestTask = await prisma.requestTask.create({
      data: {
        employeeId: faker.number.int({ min: 8, max: 16 }),
        machineId: machineId,
        faultSymptoms: faker.lorem.sentence(),
        departmentId: machine.departmentId,
        status: 'inProgress',
        requestTime: new Date(faker.date.recent(30)).toISOString(),
        updatedTime: new Date(faker.date.recent(10)).toISOString(),
        isAssigned: true,
      },
    });

    for (let j = 0; j < faker.number.int({ min: 1, max: 2 }); j++) {
      const startTime = new Date(faker.date.recent(30));
      const endTime = new Date(startTime.getTime() + 2 * 24 * 60 * 60 * 1000);
      const finishTime = new Date(startTime.getTime() + Math.random() * (endTime.getTime() - startTime.getTime()));
      const acceptTime = new Date(finishTime.getTime() + Math.random() * (2 * 24 * 60 * 60 * 1000));
      ...
      await prisma.maintenanceTask.create({
        data: {
          ...
          rejectReason: faker.datatype.boolean() ? faker.lorem.sentence() : null,
        },
      });
    }
  }
